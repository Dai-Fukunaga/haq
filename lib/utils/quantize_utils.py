...
class QModule(nn.Module):
    def __init__(self, w_bit: int = -1, a_bit: int = -1, half_wave=True):
        super(QModule, self).__init__()

        # Half wave=Trueの時は全波 (負の値から正の値まで), Falseの時は半波 (0から正の値まで)
        if half_wave:
            self._a_bit: int = a_bit
        else:
            self._a_bit: int = a_bit - 1
        self._w_bit: int = w_bit
        self._b_bit: int = 32
        self._half_wave: bool = half_wave

        self.init_range: float = 6.0
        ...
        self._quantized: bool = True
        self._tanh_weight: bool = False
        self._fix_weight: bool = False
        self._trainable_activation_range: bool = True
