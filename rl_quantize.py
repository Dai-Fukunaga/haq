    ...
    while episode < num_episode:  # counting based on episode
        ...
        if done:  # end of episode
            ...
            if linear_quantization:
                if debug:
                    print(
                        "#{}: episode_reward:{:.4f} acc: {:.4f}, cost: {:.4f}".format(
                            episode,
                            episode_reward,
                            info["accuracy"],
                            info["cost"] * 1.0 / 8e6,
                ...
                if debug:
                    print(
                        "#{}: episode_reward:{:.4f} acc: {:.4f}, weight: {:.4f} MB".format(
                            episode,
                            episode_reward,
                            info["accuracy"],
                            info["w_ratio"] * 1.0 / 8e6,
                        )
                    )
                text_writer.write(
...
if __name__ == "__main__":
    ...
    # env
    parser.add_argument(
        "--dataset", default="imagenet", type=str, help="dataset to use"
    )
    parser.add_argument(
        "--dataset_root", default="data/imagenet", type=str, help="path to dataset"
    )
    parser.add_argument(
        "--preserve_ratio",
        default=0.1,
    ...
    parser.add_argument(
        "--float_bit", default=32, type=int, help="the bit of full precision float"
    )
    parser.add_argument(
        "--linear_quantization", dest="linear_quantization", action="store_true"
    )
    ...
    parser.add_argument(
        "--lr_c", default=1e-3, type=float, help="learning rate for actor"
    )
    parser.add_argument(
        "--lr_a", default=1e-4, type=float, help="learning rate for actor"
    )
    parser.add_argument(
        "--warmup",
        default=20,
    ...
    parser.add_argument("--discount", default=1.0, type=float, help="")
    parser.add_argument("--bsize", default=64, type=int, help="minibatch size")
    parser.add_argument(
        "--rmsize", default=128, type=int, help="memory size for each layer"
    )
    parser.add_argument("--window_length", default=1, type=int, help="")
    parser.add_argument(
        "--tau", default=0.01, type=float, help="moving average for target network"
    )
    ...
    parser.add_argument(
        "--delta_decay", default=0.99, type=float, help="delta decay during exploration"
    )
    parser.add_argument(
        "--n_update", default=1, type=int, help="number of rl to update each time"
    )
    # training
    ...
    parser.add_argument(
        "--train_episode", default=600, type=int, help="train iters each timestep"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--n_worker", default=32, type=int, help="number of data loader worker"
    )
    parser.add_argument(
        "--data_bsize", default=256, type=int, help="number of data batch size"
    )
    parser.add_argument("--finetune_epoch", default=1, type=int, help="")
    parser.add_argument(
        "--finetune_gamma", default=0.8, type=float, help="finetune gamma"
    )
    parser.add_argument(
        "--finetune_lr", default=0.001, type=float, help="finetune gamma"
    )
    parser.add_argument(
        "--finetune_flag", default=True, type=bool, help="whether to finetune"
    )
    parser.add_argument(
        "--use_top5", default=False, type=bool, help="whether to use top5 acc in reward"
    )
    parser.add_argument(
        "--train_size", default=20000, type=int, help="number of train data size"
    )
    parser.add_argument(
        "--val_size", default=10000, type=int, help="number of val data size"
    )
    parser.add_argument(
